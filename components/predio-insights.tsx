...
export function PredioInsights({
  lat,
  lng,
  areaTotalHa,
  areaProductivaHa,
  ...
  const [errorNdvi, setErrorNdvi] = useState(false)
  ...
  useEffect(() => {
    ...
    fetch(`/api/ndvi?lat=${lat}&lng=${lng}`)
      ...
      .catch(() => setErrorNdvi(true))
      .finally(() => setLoadingNdvi(false))
  }, [lat, lng])

  // Si falló, no renderizar bloque NDVI (sin mensaje de error).
  const mostrarNdvi = loadingNdvi || (!errorNdvi && !!ndvi)
  ...
  return (
    <div className="space-y-3">
      ...
      {/* NDVI — oculto si falla o no hay datos */}
      {mostrarNdvi && (
        <div className="rounded-lg border border-border/50 bg-muted/20 p-3">
          <div className="flex items-center gap-2 mb-2">
